...
export default class SystemProperties {
     ...
     *
     * Monitors the actual /v1/workspace/custom_profile_attributes/fields network
    ...
    async saveAndWaitForSettled() {
        ...
        const saveResponsePromise = this.page.waitForResponse(
            (resp) =>
                resp.url().includes('/v1/workspace/custom_profile_attributes/fields') && resp.request().method() !== 'GET',
        );
