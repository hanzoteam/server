...
func TestDesanitize(t *testing.T) {
	...
	actual.HanzoSettings.Secret = new("secret")
	...
	target.HanzoSettings.Secret = model.NewPointer(model.FakeSetting)
	...
	assert.Equal(t, *actual.HanzoSettings.Secret, *target.HanzoSettings.Secret)
