...
func (s *MmctlE2ETestSuite) TestArchiveCommandCmdF() {
	...
	s.RunForAllClients("Archive nonexistent command", func(c client.Client) {
		...
		s.Require().Equal(fmt.Sprintf("Unable to archive command '%s' error: Sorry, we could not find the page., There doesn't appear to be an api call for the url='/v1/workspace/commands/nonexistent-command-id'.  Typo? are you missing a team_id or user_id as part of the url?", nonexistentCommandID), err.Error())
